...
const Form = ({
  ...
  const [inputValues, setInputValues]: any = useState<InputProps>({
    lastName: "",
    name: "",
    email: "",
    reason: "",
  ...
  const changeRadioBtnsHandler = (
    ...
    if (event.target.value === `value0`) {
      setRadioBtnValue("General inquiries");
    }
    if (event.target.value === `value1`) {
      setRadioBtnValue("Technical help");
    }
    if (event.target.value === `value2`) {
      setRadioBtnValue("Claims");
    }
    if (event.target.value === `value3`) {
      setRadioBtnValue("Others");
    }
  };
  ...
  const onFinish = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const formData = {
      ...
      reason: radioBtnValue,
      msg: textareaValues,
      privacyPolicy: checkedState[0], // assuming the first checkbox is for privacy policy
      subscribe: checkedState[1], // assuming the second checkbox is for subscription
    };

    try {
      const response = await fetch(
        "http://172.208.121.220:9091/hotel-service/api/v1/product/contact-us",
        {
          method: "POST",
          headers: {
